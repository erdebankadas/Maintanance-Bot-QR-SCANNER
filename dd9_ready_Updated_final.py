...
class QRCodeGeneratorApp:
    ...
    def __init__(self, root):
        ...
        # ── Stores path of uploaded Excel for REPRINT IN BULK ──
        self.bulk_excel_path = None
    ...
    def _build_ui(self):
        ...
        # ── BULK: upload button + status label (created once, shown/hidden) ──
        self.bulk_frame = tk.Frame(self.dynamic_frame, bg=BG_MAIN)

        self.btn_upload_excel = HoverButton(
            self.bulk_frame,
            n_bg="#1a1a00", n_fg="#ffff00",
            h_bg="#ffff00", h_fg="#000000",
            text="📂   UPLOAD EXCEL FILE",
            font=("Courier New", 9, "bold"),
            pady=8,
            command=self._upload_bulk_excel,
            highlightthickness=1,
            highlightbackground="#ffff00",
            highlightcolor="#ffff00"
        )
        self.btn_upload_excel.pack(fill="x")

        self.bulk_file_label = tk.Label(
            self.bulk_frame,
        ...
        self.bulk_file_label.pack(fill="x", pady=(4, 0))
    ...
    def option_changed(self, event=None):
        ...
        elif selected == "REPRINT IN BULK":

            # Reset previously uploaded file
            self.bulk_excel_path = None
            self.bulk_file_label.config(text="No file selected", fg=TEXT_DIM)
            self.bulk_frame.pack(fill="x", pady=(0, 10))

    # =========================================================
    # UPLOAD BULK EXCEL  ── NEW
    # =========================================================

    def _upload_bulk_excel(self):
        """Let user pick an Excel file containing machine numbers."""

        path = filedialog.askopenfilename(
            title="Select Excel File with Machine Numbers",
            filetypes=[
        ...
        self.bulk_excel_path = path

        # Show just the filename (not full path) for readability
        filename = os.path.basename(path)
        self.bulk_file_label.config(
            text=f"✔  {filename}",
            fg=ACCENT_GREEN
        )

        self.status_var.set(f"●  EXCEL LOADED  →  {filename}")

    # =========================================================
    # READ MACHINE NUMBERS FROM EXCEL  ── NEW
    # =========================================================

    def _read_machine_numbers_from_excel(self, path):
        """
        Read machine numbers from an Excel file.
        Accepts .xlsx and .xls.
        Looks through ALL cells in the FIRST sheet and collects
        values that are exactly 8 characters long (same validation
        as 'Any Particular Machine').
        Returns a list of stripped string machine numbers.
        """
        ...
        try:
            ...
            # ── handle both .xlsx and legacy .xls ──
            ...
            if ext == ".xls":
                ...
                for row_idx in range(ws.nrows):
                    for col_idx in range(ws.ncols):
                        cell_val = str(ws.cell_value(row_idx, col_idx)).strip()
                        # xlrd may return floats for numeric cells (e.g. 12345678.0)
                        if cell_val.endswith(".0"):
                            cell_val = cell_val[:-2]
                        if len(cell_val) == 8 and cell_val != "":
                            machine_numbers.append(cell_val)
            else:
                ...
                for row in ws.iter_rows(values_only=True):
                    for cell_val in row:
                        ...
                        # openpyxl may return floats stored as numbers
                        if val.endswith(".0"):
                            val = val[:-2]
                        if len(val) == 8 and val != "":
                            machine_numbers.append(val)
                wb.close()
    ...
    def process_data(self):
        ...
        try:
            ...
            # ── REPRINT IN BULK ──
            elif selected_option == "REPRINT IN BULK":

                if not self.bulk_excel_path:
                    messagebox.showwarning(
                        "No File",
                        "Please upload an Excel file containing machine numbers first."
                    )
                    return

                machine_list = self._read_machine_numbers_from_excel(
                    self.bulk_excel_path
                )

                if not machine_list:
                    messagebox.showwarning(
                        "No Valid Machine Numbers",
                        "No 8-character machine numbers found in the uploaded Excel file."
                    )
                    return

                # Build IN clause with parameterised-style quoted values
                codes_sql = ", ".join(f"'{m}'" for m in machine_list)
