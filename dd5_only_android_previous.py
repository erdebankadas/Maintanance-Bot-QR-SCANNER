...
class QRCodeGeneratorApp:
        ...
        if selected == "Particular Department":
            ...
            try:
                ...
                for row in dept_rows:
                    ...
                    dept_name = self.dept_map.get(
                        dept_code,
                        "UNKNOWN"
                    )

                    dept_display_list.append(
                        f"{dept_code} ({dept_name})"
                    )
    ...
    def generate_pdf(self, data_rows, unit_name):
        ...
        for i, record in enumerate(data_rows):
            ...
            # QR SCAN CONTENT
            # ---------------------------------------------------------
            # Plain text only — no HTML, no button.
            #
            # Layout when scanned:
            #   1. Machine info block  (clean, labelled rows)
            #   2. Divider
            #   3. WhatsApp URL on its own line
            #
            # Every modern phone scanner (Google Lens, iPhone camera,
            # Samsung scanner) auto-detects a bare https:// URL and
            # renders it as a tappable blue link right in the result
            # screen — the user taps it and WhatsApp opens with
            # "Hi" already typed, ready to send to +91 97753 30074.
            # =========================================================
            ...
            scan_content = (
                ...
                f"--------------------------------\n"
                f"\n"
                #f"https://wa.me/919775330074?text=Hi"
                f"https://wa.me/919775330074?text={code}"
            )
